...
export type StateUpdateType = (typeof StateUpdateType)[keyof typeof StateUpdateType];
...
> {
  ...
  private handleAgentConfigUpdate(data: AgentConfigUpdateEvent): void {
    ...
    if (!agentConfigVersions) {
      throw new Error(`Config versions not found for agent pool type: ${agentType}`);
    }
  ...
  private handleAgentConfigDestroy(data: AgentConfigDestroyEvent): void {
    ...
    if (!this.state.agentConfigs.has(agentType)) {
      throw new Error(`Agent pool type: ${agentType} was not found for destruction`);
    }
  ...
  private handlePoolChange(data: AgentPoolChangeEvent): void {
    ...
    const pool = this.state.agentPools.get(agentSomeIdToKindValue(agentTypeId) as AgentKindEnum);
    if (!pool) {
  ...
  private handleAgentLifecycle(
    data: AgentCreateEvent | AgentAcquireEvent | AgentReleaseEvent | AgentDestroyEvent,
  ): void {
    const { agentId } = data;
  ...
  private handleAssignmentAssign(data: TaskAssignedEvent): void {
    const { agentId, assignmentId, assignmentKind, assignedSince, assignment } = data;
    ...
    if (agent.assignments.has(assignmentId)) {
      throw new Error(`Assignment ${assignmentId} already exists for agent ${agentId}`);
    }
  ...
  private handleAssignmentUnassign(data: TaskUnassignedEvent): void {
    ...
    if (!agent.assignments.has(assignmentId)) {
      throw new Error(`Assignment ${assignmentId} not found for agent ${agentId}`);
    }
  ...
  getAgentConfig(agentTypeId: string, agentVersion?: number): AgentConfig | undefined {
  ...
  private getAgentPoolsMap(agentKindId: AgentKindEnum): Map<string, AgentPool> | undefined {
    return this.state.agentPools.get(agentKindId);
  }

  getAgentPool(agentKind: AgentKindEnum, agentType: string): AgentPool | undefined {
  ...
  getAgentAssignment(agentId: string, assignmentId: string): Assignment | undefined {
    const agent = this.state.agents.get(agentId);
