...
export class TaskStateBuilder extends BaseStateBuilder<typeof TaskStateDataTypeSchema, TaskState> {
  constructor() {
  ...
  private handleTaskRunLifecycle(
    ...
    switch (data.kind) {
      ...
      case "task_run_update": {
        ...
        if (!taskRunInfo) {
          throw new Error(`Task run info ${taskRunId} doesn't exist for update`);
        }
      ...
      case "task_run_destroy": {
        const taskRunInfo = this.state.taskRuns.get(taskRunId);
        if (!taskRunInfo) {
          throw new Error(`Task run info ${taskRunId} doesn't exist for destroy`);
        }
  ...
  handlePoolChange(data: TaskPoolChangeEvent) {
    ...
    const pool = this.state.taskRunPools.get(taskSomeIdToKindValue(taskTypeId) as TaskKindEnum);
    if (!pool) {
  ...
  handleTaskConfigUpdate(data: TaskConfigUpdateEvent) {
    ...
    if (!taskConfigsVersions) {
      throw new Error(`Task config versions not found for task type: ${taskTypeId}`);
    }
  ...
  getTaskConfig(taskTypeId: string, taskConfigVersion?: number): TaskConfig | undefined {
  ...
  private getTaskPoolsMap(agentKindId: TaskKindEnum): Map<string, TaskPool> | undefined {
    return this.state.taskRunPools.get(agentKindId);
  }

  getTaskPool(agentKind: TaskKindEnum, agentType: string): TaskPool | undefined {
