...
export const TaskKindValueSchema = z
  .string()
  .refine((value) => Object.values(TaskKindEnumSchema.enum).includes(value as TaskKindEnum), {
...
export const TaskTypeValueSchema = z.string().describe("Unique short name of the task type.");
export type TaskTypeValue = z.infer<typeof TaskTypeValueSchema>;

export const TaskRunNumValueSchema = z.number().describe("Task run instance number.");
...
export const TaskConfigVersionValueSchema = z.number().describe("Task config version number.");
export type TaskConfigVersionValue = z.infer<typeof TaskConfigVersionValueSchema>;
...
export type TaskConcurrencyModeEnum = z.infer<typeof TaskConcurrencyModeEnumSchema>;
...
export const TaskConfigSchema = z
  .object({
    ...
    taskConfigVersion: z.number().describe("Version number for the task configuration"),
    taskConfigInput: z
      .string()
      .describe(
...
export const TaskRunSchema = z
  .object({
    ...
    startRunAt: DateStringSchema.optional().describe("Timestamp of the execution start."),
    lastRunAt: DateStringSchema.optional().describe("Timestamp of the last successful execution"),
    ...
    currentRetryAttempt: z
      .number()
      .describe("Current retry count. Maximum retries configured via maxRepeats"),
    ownerAgentId: AgentIdValueSchema.describe("ID of the agent who owns/manages this task"),
    currentAgentId: z
      .string()
      .optional()
      .nullable()
    ...
    history: z.array(TaskRunHistoryEntrySchema).describe("History of task executions"),
    maxHistoryEntries: z
      .number()
      .optional()
...
export const isTaskRunActiveStatus = (status: TaskRunStatusEnum): status is ActiveStatus =>
...
): status is TerminationStatus => TERMINATION_STATUSES.includes(status as TerminationStatus);
