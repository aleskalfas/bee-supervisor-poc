...
export class PlatformSdk {
  ...
  async listAgents() {
    ...
    return agents.agents
      .filter((agent) => !this.availableAgents || this.availableAgents.includes(agent.name))
      .map((agent) => ({
  ...
  async runAgent(beeAiAgentId: string, prompt: string) {
    ...
    if (!agents.map(({ beeAiAgentId }) => beeAiAgentId).includes(beeAiAgentId)) {
      throw new Error(`Agent ${beeAiAgentId} is not registered in the platform`);
    }

    return this.client.runAgent({ name: beeAiAgentId, input: { prompt } }, { timeout: 10000000 });
  }
}
