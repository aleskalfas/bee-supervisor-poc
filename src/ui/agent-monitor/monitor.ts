...
export class AgentMonitor extends BaseMonitor {
  ...
  private updateAgentPoolsList(shouldRender = true): void {
    ...
    Array.from(state.agentPools.entries())
      .sort(([a], [b]) => {
        ...
        const aSuper = aPoolId.agentKind === AgentKindEnumSchema.Values.supervisor;
        const bSuper = bPoolId.agentKind === AgentKindEnumSchema.Values.supervisor;
        if (aSuper && !bSuper) {
          return -1;
      ...
      .forEach(([agentKindStr, agentTypePools]) => {
        ...
        Array.from(agentTypePools.entries())
          ...
          .forEach(([agentTypeStr, agentPool]) => {
            this.agentPoolListItemsData.push({
              agentTypeId: { agentKind: agentKindId.agentKind, agentType: agentTypeStr },
  ...
  private updateAgentVersionsList(shouldRender = true): void {
          ...
          clone(agentPool.versions)
            ...
            .forEach(([agentConfigVersion, poolStats]) => {
              ...
              this.agentVersionsListItemsData.push({
                agentTypeId,
                itemContent: st.versionAgentPoolStats(st.versionNum(agentConfigVersion), poolStats),
              });
            });
        }
      }
    ...
    this.agentVersionsList.setItems(this.agentVersionsListItemsData.map((it) => it.itemContent));
    ...
    if (this.agentVersionsListSelectedIndex != null) {
      Array.from(this.stateBuilder.getAllAgents())
        .filter((a) => {
          ...
          if (agentPoolListItem && agentPoolListItem.agentTypeId.agentKind === agentId.agentKind) {
            if ((agentPoolListItem.agentTypeId as AgentTypeId).agentType != null) {
              if ((agentPoolListItem.agentTypeId as AgentTypeId).agentType == agentId.agentType) {
                if ((agentPoolListItem.agentTypeId as AgentConfigId).agentConfigVersion != null) {
                  return (
                    (agentPoolListItem.agentTypeId as AgentConfigId).agentConfigVersion ===
                    agentId.agentConfigVersion
                  );
                }
              } else {
                return false;
        ...
        .sort((a, b) => {
          ...
          if (aAgentId.agentConfigVersion != bAgentId.agentConfigVersion) {
            return Math.sign(aAgentId.agentConfigVersion - bAgentId.agentConfigVersion);
          }
