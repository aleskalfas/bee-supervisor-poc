...
> extends EventEmitter {
  ...
  public on<K extends keyof StateBuilderEvents>(event: K, listener: StateBuilderEvents[K]): this {
  ...
  public async watchLogFile(filePath: string): Promise<void> {
    ...
    try {
      ...
      // Set up file watcher
      this.watcher = watch(filePath, { persistent: true }, async (eventType) => {
        if (eventType === "change") {
      ...
      if (error instanceof Error) {
        this.emit("error", new Error(`Failed to start file watching: ${error.message}`));
      }
      throw error;
    }
  }
  ...
  private async processEntireFile(filePath: string): Promise<void> {
    try {
      ...
      if (error instanceof Error) {
        this.emit("error", new Error(`Failed to process entire file: ${error.message}`));
      }
      throw error;
    }
  }
  ...
  private async processFileChanges(filePath: string): Promise<void> {
    try {
      ...
      if (error instanceof Error) {
        this.emit("error", new Error(`Failed to process file changes: ${error.message}`));
      }
      throw error;
    }
  }
  ...
  private async processLogLine(line: string): Promise<void> {
    ...
    try {
      ...
      this.emit("error", new Error(`Invalid log entry - neither init nor update: ${line}`));
    } catch (error) {
      if (error instanceof Error) {
        this.emit("error", new Error(`Failed to process log line: ${error.message}`));
      }
    }
  }
