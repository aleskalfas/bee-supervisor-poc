...
export class ResourcesAccessControl {
  ...
  private registry: ResourcesPermissionRegistry = {
    admins: new Set(),
    resources: new Map([[REGISTRY_RESOURCE, { ownerId: ADMIN_USER, userPermissions: new Map() }]]),
  };
  ...
  private checkReservedConstants(...requestedIds: (UserId | ResourceId)[]) {
    RESERVED_IDS.forEach((id) => {
      requestedIds.forEach((requestedId) => {
        if (requestedId === id) {
          throw new Error(`${requestedId} is reserved for system purpose please chose another`);
        }
      });
    });
  }
  ...
  private getResourcePermissions(resourceId: ResourceId, throwError = true) {
    ...
    if (!resourcePermissions && throwError) {
      throw new Error(`Resource permissions for resourceId:${resourceId} was not found`);
    }
    return resourcePermissions;
  }

  private destroyResourcePermissions(resourceId: ResourceId, throwError = true) {
  ...
  createPermissions(
    ...
    this.logger.info({ resourceId, userId, permissions, actingUserId }, `createPermissions`);
  ...
  removePermissions(
    ...
    this.logger.info({ resourceId, userId, permissions, actingUserId }, `removePermissions`);
  ...
  createResource(
    ...
    this.logger.info({ resourceId, ownerId, actingUserId, parentResourceId }, `createResource`);
    this.checkReservedConstants(resourceId, ownerId);
    if (parentResourceId) {
      this.checkPermission(parentResourceId, actingUserId, WRITE_ONLY_ACCESS, false);
    }

    let resourcePermissions = this.getResourcePermissions(resourceId, false);
    if (resourcePermissions) {
      throw new Error(`Resource permissions for resourceId: ${resourceId} already exist`);
    }
    resourcePermissions = { ownerId, userPermissions: new Map() } satisfies ResourcePermissions;
  ...
  checkPermission(
    ...
    const resourcePermissions = this.getResourcePermissions(resourceId, throwError)!;
    ...
    requestedPermissions.forEach((permission) => {
      const userResourcePermissions = resourcePermissions.userPermissions.get(userId);
      if (!userResourcePermissions || !userResourcePermissions.includes(permission)) {
        throw new PermissionError(userId, permission, resourcePermissions.ownerId);
      }
    });
  }
